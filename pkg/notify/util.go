...
func GetTemplateData(ctx context.Context, tmpl *template.Template, alerts []*types.Alert, log *logrus.Entry) *template.Data {
	recv, ok := ReceiverName(ctx)
	if !ok {
		log.WithField("msg", "Missing receiver").Error()
	}
	groupLabels, ok := GroupLabels(ctx)
	if !ok {
		log.WithField("msg", "Missing group labels").Error()
	}
