...
func (pb *PipelineBuilder) New(
	...
	for name := range receivers {
		st := createReceiverStage(name, receivers[name], pb.metrics)
		rs[name] = MultiStage{st}
	}
	return rs
}
...
func (n *GossipSettleStage) Exec(ctx context.Context, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
	if n.peer != nil {
		n.peer.WaitReady()
	}
	return ctx, alerts, nil
...
// Exec implements the Stage interface.
func (n *MuteStage) Exec(ctx context.Context, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
...
func (r RetryStage) Exec(ctx context.Context, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
	...
	log := logging.DefaultLogger.WithFields(logrus.Fields{"receiver": r.groupName, "integration": r.integration.String()})
		...
		case <-tick.C:
			...
			if err != nil {
				...
				if ctx.Err() == nil && (iErr == nil || err.Error() != iErr.Error()) {
					...
					log.WithFields(logrus.Fields{"msg": "Notify attempt failed, will retry later", "attempts": i}).WithError(err).Warn()
				}
				...
				log.WithFields(logrus.Fields{"msg": "Notify success", "attempts": i}).Info()
				return ctx, alerts, nil
			}
		case <-ctx.Done():
			if iErr == nil {
