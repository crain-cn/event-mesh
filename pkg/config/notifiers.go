...
var (
	...
	DefaultDogConfig = DogConfig{
		NotifierConfig: NotifierConfig{
			VSendResolved: true,
		},
	}

	DefaultYachConfig = YachConfig{
		NotifierConfig: NotifierConfig{
			VSendResolved: true,
		},
		Title:   `{{ template "yach.default.title" . }}`,
		Message: `{{ template "yach.default.message" . }}`,
	}
...
type DogConfig struct {
	...
	MaxAlerts      uint64                      `yaml:"max_alerts" json:"max_alerts"`
	HTTPConfig     *commoncfg.HTTPClientConfig `yaml:"http_config,omitempty" json:"http_config,omitempty"`
	TaskId         int32                       `yaml:"task_id,omitempty"  json:"task_id,omitempty"`
...
// UnmarshalYAML implements the yaml.Unmarshaler interface.
func (c *DogConfig) UnmarshalYAML(unmarshal func(interface{}) error) error {
	*c = DefaultDogConfig
	type plain DogConfig
	if err := unmarshal((*plain)(c)); err != nil {
		return err
	}
	if c.TaskId == 0 {
		return fmt.Errorf("missing in TaskId")
	}
	return nil
}
...
type YachConfig struct {
	HTTPConfig     *commoncfg.HTTPClientConfig `yaml:"http_config,omitempty" json:"http_config,omitempty"`
	NotifierConfig `yaml:",inline" json:",inline"`
	AccessToken    string `yaml:"access_token,omitempty" json:"access_token"`
...
// UnmarshalYAML implements the yaml.Unmarshaler interface.
func (c *YachConfig) UnmarshalYAML(unmarshal func(interface{}) error) error {
	*c = DefaultYachConfig
	type plain YachConfig
	if err := unmarshal((*plain)(c)); err != nil {
		return err
	}
	if len(c.AccessToken) == 0 {
		return fmt.Errorf("missing in AccessToken")
	}
	if len(c.Secret) == 0 {
		return fmt.Errorf("scheme required secret")
	}
	return nil
}
