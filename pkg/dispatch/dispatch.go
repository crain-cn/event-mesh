...
func NewDispatcher(
	...
	disp := &Dispatcher{
		...
		timeout: to,
		metrics: m,
		logger:  logging.DefaultLogger.WithField(logfields.LogSubsys, "dispatcher"),
	}
	return disp
}
		...
		case alert, ok := <-it.Next():
			if !ok {
				...
				if err := it.Err(); err != nil {
					d.logger.WithError(err).WithField("msg", "Error on alert update")
				}
				return
			}
			d.logger.WithFields(logrus.Fields{"msg": "Received alert", "alert": alert}).Debug()
...
func (d *Dispatcher) ProcessAlert(alert *types.Alert, route *Route) {
	...
	if !ok {
		...
		go ag.run(func(ctx context.Context, alerts ...*types.Alert) bool {
			...
			if err != nil {
				lvl := d.logger.WithFields(logrus.Fields{"msg": "Notify for alerts failed", "num_alerts": len(alerts)}).WithError(err)
...
func newAggrGroup(ctx context.Context, labels model.LabelSet, r *Route, to func(time.Duration) time.Duration) *aggrGroup {
	...
	ag.logger = logging.DefaultLogger.WithField("aggrGroup", ag)
...
func (ag *aggrGroup) run(nf notifyFunc) {
	defer func(){
		if r := recover(); r != nil {
			ag.logger.WithField("msg", "Recovered in f").WithError(r.(error)).Error()
		}
	}()
	defer func(agl *aggrGroup) {
		defer func(){
			if r := recover(); r != nil {
				agl.logger.WithField("msg", "Recovered in f").WithError(r.(error)).Error()
			}
		}()
		if agl.next != nil {
			agl.next.Stop()
...
func (ag *aggrGroup) insert(alert *types.Alert) {
	if err := ag.alerts.Set(alert); err != nil {
		ag.logger.WithField("msg", "error on set alert").WithError(err).Error()
	}
...
func (ag *aggrGroup) flush(notify func(...*types.Alert) bool) {
	...
	ag.logger.WithFields(logrus.Fields{"msg": "error on set alert", "alerts": fmt.Sprintf("%v", alertsSlice)}).Debug()
	...
	if notify(alertsSlice...) {
		for _, a := range alertsSlice {
			...
			alertLog := ag.logger.WithField("alert", a.String())
			if err != nil {
				// This should never happen.
				alertLog.WithField("msg", "failed to get alert").WithError(err).Error()
				continue
			}
			if a.Resolved() && got.UpdatedAt == a.UpdatedAt {
				if err := ag.alerts.Delete(fp); err != nil {
					alertLog.WithField("msg", "error on delete alert").WithError(err).Error()
				}
			}
		}
	}
