...
route:
  ...
  - match:
	...
	dispatcher := NewDispatcher(alerts, route, recorder, marker, timeout, NewDispatcherMetrics(prometheus.NewRegistry()))
	go dispatcher.Run()
...
func (r *recordStage) Exec(ctx context.Context, alerts ...*types.Alert) (context.Context, []*types.Alert, error) {
	r.mtx.Lock()
	defer r.mtx.Unlock()
...
func TestDispatcherRace(t *testing.T) {
	...
	timeout := func(d time.Duration) time.Duration { return time.Duration(0) }
	dispatcher := NewDispatcher(alerts, nil, nil, marker, timeout, NewDispatcherMetrics(prometheus.NewRegistry()))
	go dispatcher.Run()
	dispatcher.Stop()
}
