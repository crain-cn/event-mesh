...
// List lists all Receivers in the indexer.
func (s *receiverLister) List(selector labels.Selector) (ret []*v1.Receiver, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1.Receiver))
	})
	return ret, err
}

// Get retrieves the Receiver from the index for a given name.
func (s *receiverLister) Get(name string) (*v1.Receiver, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1.Resource("receiver"), name)
	}
	return obj.(*v1.Receiver), nil
}
